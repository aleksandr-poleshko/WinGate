...
#					header('Content-Length: ' . filesize($file));
		...
		if( $_JCMS->query[0] == 'balance' && $_JCMS->query[1] == 'payment_freekassa' && !$_JCMS->query[2] ){ ob_end_clean(); $this->action_payment(); exit(); } // оплата процессинг
...
//							$table .= "<a href=\"".$url."\">Скачать</a><br>";
				...
				if( $show_testOrderNotice == 1 && !$_SESSION["JENSENCMS"]['client_msg'] ){
					unset($_SESSION["JENSENCMS"]['client_msg']); 
					if( $_JCMS->lang == 'en' ){
						$_SESSION["JENSENCMS"]['client_msg'] = array("<b>Request for test access is sent to the administrator!</b>", 'To activate a test access, please provide your username on these contacts:<br><b>Telegram:</b> <a href="https://t.me/wingateproxy" target="_blank">@wingateproxy</a><br>','warning'); 
					} else {
						$_SESSION["JENSENCMS"]['client_msg'] = array("<b>Заявка на тестовый доступ отправлена администратору!</b>", 'Для активации тестового доступа сообщите ваш логин по ниже указанным контактам:<br><b>Telegram:</b> <a href="https://t.me/wingateproxy" target="_blank">@wingateproxy</a><br>','warning'); 
					}
				}
...
*/		
		...
		if( $_JCMS->lang == 'en' ){
			$inv_desc = "Refill the balance on the WinGate service.";
		} else {
			$inv_desc = "Пополнение баланса на сервисе WinGate.";			
		}
...
//		if( $_GET['test'] == 1 ){ $_POST = $_GET; }
		...
		$client_id = $_POST["Shp_client_id"];
		$crc = $_POST["SignatureValue"];
		$crc = strtoupper($crc);
		$my_crc = strtoupper(md5("$out_summ:$invoice_id:$mrh_pass2:Shp_client_id=$client_id"));
//		if( $_GET['test'] == 1 ){ $client_id = '21'; } 
		...
		if (strtoupper($my_crc) == strtoupper($crc)){
			...
			if($res->num_rows == 1 ){
				...
				$sql_code = "INSERT INTO `jcms2_moduleClientsBalance`(`client_id`, `summ`, `descr`) VALUES ('{$client_id}', '{$out_summ}', 'Пополнение через FREE-KASSA <i>(транзакция №{$invoice_id})</i>')";
